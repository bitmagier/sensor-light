...
use crate::peripheral::PresenceSensor;
...
impl<P1: Pin, P2: Pin> Devices<P1, P2> {
    ...
    fn enable_presence_sensor(&mut self) -> Result<()> {
        // due to the nature of the PNP transistor circuit we need to set the PIN on low in order to switch it ON
    ...
    pub fn presence_sensor_enabled(&self) -> bool {
        self.presence_sensor_power_pin.is_set_low()
    }
